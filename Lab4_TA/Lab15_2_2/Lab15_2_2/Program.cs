    ...
    switch (opt)
    {
        ...
        case 2:
            ...
            for (int i = 0; i < mass_of_students.Length; i++)
            {
                ...
                {
                    writer.WriteLine($"Фамилия - {mass_of_students[i].female}, Отметка по математике - {mass_of_students[i].mark_math}, Отметка по истории - {mass_of_students[i].mark_history} , Средний балл студента - {mass_of_students[i].sredniy_bal}");
                }
            }
            break;
        case 3:
            ...
            for (int i = 0; i < res1.Length; i++)
            {
                ...
                {
                    writer.WriteLine($"Фамилия - {res1[i].female}, Отметка по математике - {res1[i].mark_math}, Отметка по истории - {res1[i].mark_history} , Средний балл студента - {res1[i].sredniy_bal}");
                }
            ...
            for (int i = 0; i < res2.Length; i++)
            {
                using (StreamWriter writer = new StreamWriter(path, true))
                {
                    writer.WriteLine($"Фамилия - {res2[i].female}, Отметка по математике - {res2[i].mark_math}, Отметка по истории - {res2[i].mark_history} , Средний балл студента - {res2[i].sredniy_bal}");
                }
                Output(res2[i]);
            }
            break;
